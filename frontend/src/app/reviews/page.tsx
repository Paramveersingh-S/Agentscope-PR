...
export default function ReviewsPage() {
  ...
  const { data: reviewsData, isLoading } = useQuery({ queryKey: ['reviews'], queryFn: () => getReviews(1) });
  
  const reviews = reviewsData?.data || [];
  
  const filteredReviews = reviews.filter((r: any) => 
    r.pr_title?.toLowerCase().includes(search.toLowerCase()) ||
    r.repository?.full_name?.toLowerCase().includes(search.toLowerCase()) ||
                ...
                <tr key={review.id} className="border-b border-border/50 last:border-0 hover:bg-accent/30 transition-colors group">
                  <td className="px-6 py-4">
                    <div className="flex items-center">
                      ...
                      <div>
                        <div className="text-white font-medium">{review.pr_title || `PR #${review.pr_number}`}</div>
                        <div className="text-xs text-muted-foreground mt-0.5">{review.repository?.full_name} #{review.pr_number}</div>
                      </div>
                    </div>
                  </td>
                  <td className="px-6 py-4">
                    ...
                    {review.status === 'success' && <span className="px-2.5 py-1 text-xs font-semibold rounded-full bg-green-500/10 text-green-500 border border-green-500/20">Success</span>}
                    {review.status === 'queued' && <span className="px-2.5 py-1 text-xs font-semibold rounded-full bg-yellow-500/10 text-yellow-500 border border-yellow-500/20">Queued</span>}
                    ...
                    {review.status === 'error' && <span className="px-2.5 py-1 text-xs font-semibold rounded-full bg-destructive/10 text-destructive border border-destructive/20">Error</span>}
                  </td>
                  <td className="px-6 py-4">
                    {review.overall_score !== null && review.overall_score !== undefined ? (
                      <div className="flex items-center">
                        <span className={`font-bold ${review.overall_score >= 8 ? 'text-green-400' : review.overall_score >= 6 ? 'text-yellow-400' : 'text-destructive'}`}>
                  ...
                  <td className="px-6 py-4">
                    {review.recommendation === 'REQUEST_CHANGES' ? (
                      <div className="flex items-center text-destructive">
                        ...
                        <span className="font-bold">Changes Req</span>
                      </div>
